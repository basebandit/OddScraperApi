<template>
  <v-container fluid grid-list-xl py-0>
    <app-card :heading="plHeader" :fullBlock="true" v-if="premierLeague">
      <v-card-title>{{plHeader}}
        <v-spacer></v-spacer>
        <v-text-field append-icon="search" label="Search Match" single-line hide-details v-model="search">
        </v-text-field>
      </v-card-title>
      <v-data-table :headers="plHeaders" :items="premierLeague.data" :search="search">
    ...
    <app-card :heading="plHeader" :fullBlock="true" v-else>
      <v-card-title>{{plHeader}}
        <v-spacer></v-spacer>
      </v-card-title>
    </app-card>
...
<script>
  ...
  export default {
    ...
    mounted () {
      this.$store.dispatch('allPremierLeagueMatches')
    },
    computed: {
      ...mapGetters({
        premierLeague: 'getPremierLeagueMatches'
      })
    }
  }
</script>
