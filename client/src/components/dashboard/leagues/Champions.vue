<template>
  <v-container fluid grid-list-xl py-0>
    <app-card :heading="chHeader" :fullBlock="true" v-if="champions">
      <v-card-title>{{chHeader}}
        <v-spacer></v-spacer>
        <v-text-field append-icon="search" label="Search Match" single-line hide-details v-model="search">
        </v-text-field>
      </v-card-title>
      <v-data-table :headers="chHeaders" :items="champions.data" :search="search">
    ...
    <app-card :heading="chHeader" :fullBlock="true" v-else>
      <v-card-title>{{chHeader}}
        <v-spacer></v-spacer>
      </v-card-title>
    </app-card>
...
<script>
  ...
  export default {
    ...
    mounted () {
      this.$store.dispatch('allChampionsMatches')
    },
    computed: {
      ...mapGetters({

        champions: 'getChampionsMatches'
