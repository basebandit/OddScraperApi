<template>
  <v-container fluid grid-list-xl py-0>
    <app-card :heading="llHeader" :fullBlock="true" v-if="laLiga">
      <v-card-title>{{llHeader}}
        <v-spacer></v-spacer>
        <v-text-field append-icon="search" label="Search Match" single-line hide-details v-model="search">
        </v-text-field>
      </v-card-title>
      <v-data-table :headers="llHeaders" :items="laLiga.data" :search="search">
    ...
    <app-card :heading="llHeader" :fullBlock="true" v-else>
      <v-card-title>{{llHeader}}
        <v-spacer></v-spacer>
      </v-card-title>
    </app-card>
...
<script>
  ...
  export default {
    ...
    mounted () {
      this.$store.dispatch('allLaLigaMatches')
    },
    computed: {
      ...mapGetters({
        laLiga: 'getLaLigaMatches'
      })
    }
  }
</script>
